        ...
        <div className="hidden xl:flex items-center gap-6">
          {Object.entries(navItems).map(([item, url]) => (
            <Link
              key={item}
              href={url}
              className="font-jost! text-sm 2xl:text-md font-medium px-2 2xl:px-4 py-2 rounded-xl transition-all duration-300 ease-out hover:bg-white/8 hover:backdrop-blur-sm hover:shadow-md hover:shadow-white/10 hover:-translate-y-0.5 hover:scale-[1.02] active:scale-[0.98]"
            >
              {item}
            </Link>
          ))}
        ...
        <div className="xl:hidden mt-12 px-8 pb-4 space-y-3">
          {Object.entries(navItems).map(([item, url]) => (
            <Link
              key={item}
              href={url}
              className="block w-full font-jost! text-left text-md font-medium px-4 py-3 rounded-xl transition-all duration-300 ease-out hover:bg-white/8 hover:backdrop-blur-sm hover:shadow-md hover:shadow-white/10"
              onClick={() => setIsMenuOpen(false)}
            >
              {item}
            </Link>
